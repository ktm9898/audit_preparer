...
3. 근거 명시: '관련 근거' 칸에는 해당 리스크가 언급된 기사의 날짜와 제목, 보고서의 페이지와 장/절 제목을 명시하세요.
...
{data_text}"""
    ...
    def analyze_personas(self, minutes_text: str) -> List[Dict]:
        """[복원] 과거 GAS 기반 고심도 전수 분석 프롬프트 (가독성 및 누락 방지 강화)"""
        logger.info(f"👤 [의원 성향 분석] 대상 텍스트 길이: {len(minutes_text)}자")
        if len(minutes_text) < 10:
            return []
            
        system_instruction = """당신은 대한민국 지방의회 행정사무감사 전문 분석 AI입니다.
[필수 분석 단계]
1단계: 문서(회의록) 전체를 훑어 발언 인물을 **전수(All Members)** 식별하십시오.
2단계: 식별된 의원의 '지역구' 정보를 문서 내에서 찾아내십시오. (못 찾으면 '지역구 미확인')
3단계: 각 인물의 핵심 발언과 성향을 '상세하고 논리적으로' 요약하십시오. (약 400~600자 내외)
   - 질문의 배경, 구체적인 수치, 비판의 논조, 재단측 답변에 대한 재반박 내용을 포함하십시오.
   - **[형식] 각 발언 주제별로 글머리 기호(-)를 사용하여 줄바꿈과 함께 구분하십시오.**
4단계: 요약된 내용을 바탕으로 실무자가 '즉시 대응' 가능한 의원별 상세 성향 리포트를 완성하십시오.
   - **핵심 감사 포인트: 번호(1., 2., 3.)를 사용하여 줄바꿈과 함께 3개 이상 제시하십시오.**

[절대 원칙]
- **누락 제로**: 회의록에 등장한 모든 의원을 각각 개별 객체로 생성하십시오. 한 명이라도 누락하거나 통합하지 마십시오.
- **디테일 유지**: '발언 요약' 항목에 회의록 내용을 최대한 상세히 복원하십시오.
- **형식 준수**: JSON 키값(의원명, 지역구, 주요 관심사, 질문 성향, 핵심 감사 포인트, 발언 요약)을 반드시 지키십시오."""

        prompt = f"""{system_instruction}

[미션] 회의록 내 발언 의원 **전수(100%)** 분석하여 의원별 상세 리포트 도출
(JSON 형식: {{"personas": [{{"의원명": "...", "지역구": "...", "주요 관심사": "...", "질문 성향": "...", "핵심 감사 포인트": "1. ...\\n2. ...\\n3. ...", "발언 요약": "- ...\\n- ..."}}]}})
...
2. 과거 질문 반복 절대 금지: 의원 성향 데이터에 있는 과거 발언은 '스타일 파악용'입니다. 기존 질문을 절대로 재탕하지 마십시오.
3. 융합적 분석: "의원 페르소나 + 출처 기반의 구체적 사안(Source Context)과 이슈(News)"를 결합하세요.
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                if isinstance(val, list):
                    return val
                # 키값의 내용이 리스트가 아닌 단일 객체라면 리스트로 감싸서 반환
                if val:
                    return [val]
                # 만약 키가 없는데 데이터 자체가 분석 결과인 경우 (예: {"의원명": ...})
                if key not in data and any(k in data for k in ["의원명", "요인", "질문"]):
                    return [data]
            return []
        except Exception as e:
