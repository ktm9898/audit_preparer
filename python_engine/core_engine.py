...
[지시사항]
1. 전수 조사 (최우선): 회의록 데이터에 등장하여 발언한 **모든 의원을 단 한 명도 누락하지 말고** 각각 분석하여 리스트에 포함하세요.
2. 실증적 분석: '친절함', '날카로움' 같은 주관적 표현보다 "ㅇㅇ사업의 예산 낭비를 강하게 질타함"과 같이 구체적인 발언 맥락을 기술하세요.
3. 요약의 미학: 분석할 의원이 많을 수 있으므로, 각 항목(발언요약, 감사 포인트)은 2문장 내외로 핵심만 아주 짧게 요약하여 전체 응답이 JSON 형식을 유지하게 하세요.
4. 데이터 충실도: 제공된 회의록 원문에 등장하는 특정 사업명이나 키워드를 반드시 포함하세요.
