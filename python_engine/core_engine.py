...
- **디테일 유지**: '발언요약' 항목은 이 시스템의 핵심입니다. 회의록 내용을 최대한 많이 복원하여 상세히 적으십시오. 소설을 쓰지 말고 '사실에 기반한 디테일'을 채우십시오. **각 발언 주제별로 번호(1., 2., 3.)를 매겨 줄바꿈과 함께 구분하십시오.**
- **입력 규모 대응**: 분석해야 할 의원이 많을 경우, 개별 요약의 길이를 핵심 위주로 소폭 조정하더라도 **인원수를 줄여서는 절대 안 됩니다.**
- '핵심 감사 포인트'는 의원의 과거 질문 패턴을 분석해 이번 감사에서 공격해올 구체적인 아킬레스건을 **글머리 기호(-)를 사용하여 3개 이상 줄바꿈으로 구분해 제시하십시오.**"""
